...
const Proyecto = () => {
    ...
    let tareasFiltradas = proyecto.tareas?.filter(tareaState => tareaState.prioridad===filtroPrioridad)
    if (filtroPrioridad === 'todas'){
        tareasFiltradas = proyecto.tareas
    }
          
     let tareasFiltradasEstado = tareasFiltradas?.filter(tarea => tarea.estado.toString()==filtroEstado)
    if (filtroEstado === 'todas'){
        tareasFiltradasEstado = tareasFiltradas
    }
        ...
        <>
            {cargando ? <Spinner /> :
                <>
                    ...
                    <div className='mt-5 flex flex-col gap-2 md:flex-row md:items-center md:justify-around'>
                        <p className='font-bold text-xl  text-gray-800'>Tareas del Proyecto</p>
                        <select
                            onChange={e=>setFiltroEstado(e.target.value)}
                             
                            className='border-x rounded-lg px-2 py-2'>
                    ...
                    <div className=' border-2 bg-white shadow mt-10 rounded-lg  overflow-y-scroll scrollbar-thumb-transparent scrollbar-thin
                                        hover:scrollbar-thumb-sky-700 scrollbar-thumb-rounded-full h-96 transition-colors'>
                        {proyecto.tareas?.length ? tareasFiltradasEstado.map(tarea => (
